...
function automatorwp_zoom_settings_sections( $automatorwp_settings_sections ) {
    ...
    $automatorwp_settings_sections['zoom'] = array(
        'title' => __( 'Zoom', 'automatorwp-zoom' ),
...
function automatorwp_zoom_settings_meta_boxes( $meta_boxes )  {
    ...
    $meta_boxes['automatorwp-zoom-meetings-settings'] = array(
        'title' => automatorwp_dashicon( 'groups' ) . __( 'Zoom Meetings', 'automatorwp-zoom' ),
        'fields' => apply_filters( 'automatorwp_zoom_meetings_settings_fields', array(
            $prefix . 'meetings_client_id' => array(
                'name' => __( 'Client ID:', 'automatorwp-zoom' ),
                'desc' => __( 'Your Zoom app client ID.', 'automatorwp-zoom' ),
                'type' => 'text',
            ),
            $prefix . 'meetings_client_secret' => array(
                'name' => __( 'Client Secret:', 'automatorwp-zoom' ),
                'desc' => __( 'Your Zoom app client secret.', 'automatorwp-zoom' ),
                'type' => 'text',
            ),
...
function automatorwp_zoom_redirect_url_display_cb( $field_args, $field ) {
    ...
    <div class="cmb-row cmb-type-custom cmb2-id-automatorwp-zoom-redirect-url table-layout" data-fieldtype="custom">
        <div class="cmb-th">
            <label><?php echo __( 'Redirect and Whitelist URL:', 'automatorwp-zoom' ); ?></label>
        </div>
        <div class="cmb-td">
            <input type="text" class="regular-text" value="<?php echo $admin_url; ?>" readonly>
            <p class="cmb2-metabox-description"><?php echo __( 'Copy this URL and place it on your Zoom app redirect and whitelist URL fields.', 'automatorwp-zoom' ); ?></p>
        </div>
    </div>
    <?php
}
...
function automatorwp_zoom_authorize_display_cb( $field_args, $field ) {
    ...
    <div class="cmb-row cmb-type-custom cmb2-id-automatorwp-zoom-authorize table-layout" data-fieldtype="custom">
        <div class="cmb-th">
            <label><?php echo __( 'Connect with Zoom:', 'automatorwp-zoom' ); ?></label>
        </div>
        <div class="cmb-td">
            <a id="<?php echo $field_id; ?>" class="button button-primary" href="#"><?php echo __( 'Authorize', 'automatorwp-zoom' ); ?></a>
            <p class="cmb2-metabox-description"><?php echo __( 'Add you Zoom app client ID and secret fields and click on "Authorize" button to generate access keys for this site.', 'automatorwp-zoom' ); ?></p>
            <?php if ( is_array( $auth ) ) : ?>
                <div class="automatorwp-notice-success"><?php echo __( 'Site connected with Zoom successfully.', 'automatorwp-zoom' ); ?></div>
                <p class="automatorwp-zoom-access-token"><strong><?php echo __( 'Access token:', 'automatorwp-zoom' ); ?></strong> <input type="text" value="<?php echo $auth['access_token']; ?>" readonly></p>
                <p class="automatorwp-zoom-refresh-token"><strong><?php echo __( 'Refresh token:', 'automatorwp-zoom' ); ?></strong> <input type="text" value="<?php echo $auth['refresh_token']; ?>" readonly></p>
            <?php elseif( ! empty( $client_id ) && ! empty( $client_secret ) ) : ?>
                <div class="automatorwp-notice-error"><?php echo __( 'Site not connected with Zoom.', 'automatorwp-zoom' ); ?></div>
            <?php endif; ?>
        </div>
    </div>
    <?php
