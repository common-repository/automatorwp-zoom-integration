...
class AutomatorWP_Zoom_Add_User_Meeting extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Add user to meeting', 'automatorwp-zoom' ),
            'select_option'     => __( 'Add user to <strong>meeting</strong>', 'automatorwp-zoom' ),
            /* translators: %1$s: Meeting. */
            'edit_label'        => sprintf( __( 'Add user to %1$s', 'automatorwp-zoom' ), '{meeting}' ),
            /* translators: %1$s: Meeting. */
            'log_label'         => sprintf( __( 'Add user to %1$s', 'automatorwp-zoom' ), '{meeting}' ),
            'options'           => array(
                'meeting' => automatorwp_utilities_ajax_selector_option( array(
                    'field'             => 'meeting',
                    'option_default'    => __( 'meeting', 'automatorwp-zoom' ),
                    'placeholder'       => __( 'Select a meeting', 'automatorwp-zoom' ),
                    'name'              => __( 'Meeting:', 'automatorwp-zoom' ),
    ...
    public function execute( $action, $user_id, $action_options, $automation ) {
        ...
        if( empty( $meeting_id ) ) {
            $this->result = __( 'Could not add registrant to meeting, missing meeting', 'automatorwp-zoom' );
            return;
        }
        ...
        if( $params === false ) {
            $this->result = __( 'Could not add registrant to meeting, Zoom authentication failed', 'automatorwp-zoom' );
            return;
        }
        ...
        if ( is_wp_error( $response ) ) {
            $this->result = sprintf( __( 'Could not add registrant to meeting, error received: %1$s', 'automatorwp-zoom' ), $response->get_error_message() );
            return;
        }
        ...
        if ( $response_code === 201 ) {
            ...
            if ( isset( $body['join_url'] ) ) {
                ...
                $this->result = __( 'Registrant successfully registered', 'automatorwp-zoom' );
            }
        } else {
            $this->result = sprintf( __( 'Could not add registrant to meeting, error received: %1$s', 'automatorwp-zoom' ), $body['message'] );
        }
    ...
    public function configuration_notice( $object, $item_type ) {
        ...
        if( $params === false ) : ?>
            <div class="automatorwp-notice-warning" style="margin-top: 10px; margin-bottom: 0;">
                <?php echo sprintf(
                        __( 'You need to configure the <a href="%s" target="_blank">Zoom Meetings settings</a> to get this action to work.', 'automatorwp-zoom' ),
                        get_admin_url() . 'admin.php?page=automatorwp_settings&tab=opt-tab-zoom'
                ); ?>
                <?php echo sprintf(
                    __( '<a href="%s" target="_blank">Documentation</a>', 'automatorwp-zoom' ),
    ...
    public function log_fields( $log_fields, $log, $object ) {
        ...
        $log_fields['registrant_first_name'] = array(
            'name' => __( 'Registrant First Name:', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['registrant_last_name'] = array(
            'name' => __( 'Registrant Last Name:', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['registrant_email'] = array(
            'name' => __( 'Registrant Email:', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['meeting_id'] = array(
            'name' => __( 'Meeting ID:', 'automatorwp-zoom' ),
            'desc' => __( 'Unique identifier for the meeting.', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['registrant_id'] = array(
            'name' => __( 'Registrant ID:', 'automatorwp-zoom' ),
            'desc' => __( 'Unique identifier assigned to the registrant.', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['join_url'] = array(
            'name' => __( 'Join URL:', 'automatorwp-zoom' ),
            'desc' => __( 'Unique URL for this registrant to join the meeting.', 'automatorwp-zoom' ),
            'type' => 'text',
        );

        $log_fields['result'] = array(
            'name' => __( 'Result:', 'automatorwp-zoom' ),
            'type' => 'text',
        );
